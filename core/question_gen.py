...
async def evaluate_answer(
    ...
    user_content = f"""Верни ТОЛЬКО валидный JSON без markdown и пояснений.

Позиция: {grade} {role}
Вопрос: {question}
Ответ кандидата: {answer}

{{"score": <0-100>, "feedback": "<{detail_level} фидбек: разбери ответ по существу — что раскрыто, чего не хватает, как именно усилить с конкретным примером. Не начинай с 'Хорошо что' или похожих шаблонов. Говори прямо и по делу, как опытный тех-лид после мок-интервью>", "ideal_answer": "<2-3 конкретных тезиса которые стоило упомянуть>"}}

Тон: прямой, конкретный, без воды и лести. Разные начала фраз каждый раз."""
